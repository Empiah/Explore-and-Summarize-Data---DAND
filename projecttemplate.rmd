...
grid.arrange(qplot(x = LoanOriginalAmount, 
                   data = subset(df,  BorrowerState == 'CT'), ylab = 'Count for State = Connecticut', 
                   binwidth=1000),
             qplot(x = LoanOriginalAmount, 
                   data = subset(df,  BorrowerState == 'WV'), ylab = 'Count for State = West Virginia', 
...
> The above gives us some insight as to the size of loans people take out if they own homes, and if they do not. We can see that non-homeowners take a out more smaller loans whilst people who own a home take out more. The inflection point here is around 10,000.
...
chart1 <- ggplot(subset(df,  IncomeRange != 'Not displayed'), aes(x=LoanOriginalAmount, fill=factor(IncomeRange))) + 
  geom_density(alpha=0.2) + 
...
> This plot shows the Loan Amount of differing Income ranges. I think that this plot shows the habits or various Loanee's well as you can understand that that with a lower income may use small loans to pay for things like bills or unexpected accidents, as they may not have much saved to cover these costs. It also shows the general borrowing habits of higher earnings, who cannot only borrow more, but may be likely to be using the resulting loan to purchase more expensive goods.
...
chart3 <- ggplot(subset(df3,  BorrowerState == 'DC' & CreditScoreRangeLower > 500), 
       aes(x=LoanOriginalAmount, y=CreditScoreRangeLower,
           size=ProsperRating..numeric.,
           color=factor(IncomeRange))) + 
